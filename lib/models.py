...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, volkshashd):
        golist = volkshashd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_volkshashd(volkshashd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_volkshashd(self, volkshashd, rec):
        import decimal
        import volkshashlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from volkshashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from volkshashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from volkshashd - Volkshashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(volkshashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from volkshashd! %s" % e)
            govobj.vote_delete(volkshashd)
            return (govobj, None)
    ...
    def vote_delete(self, volkshashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(volkshashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, volkshashd, signal, outcome):
        import volkshashlib
        ...
        output = volkshashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = volkshashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(volkshashd, signal)

    def sync_network_vote(self, volkshashd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = volkshashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 VolkshashCoin addr, non-multisig
            if not volkshashlib.is_valid_volkshash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid VolkshashCoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Volkshash Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (volkshashlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not volkshashlib.is_valid_volkshash_address(addr, config.network):
    ...
    def hash(self):
        import volkshashlib
        return volkshashlib.hashit(self.serialise())
