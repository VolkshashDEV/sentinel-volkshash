...
sentinel_cfg = VolkshashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_volkshashd_proto_version_with_sentinel_ping = 70207


def get_volkshash_conf():
    if sys.platform == 'win32':
        volkshash_conf = os.path.join(os.getenv('APPDATA'), "VolkshashCore/volkshash.conf")
    else:
        home = os.environ.get('HOME')

        volkshash_conf = os.path.join(home, ".volkshashcore/volkshash.conf")
        if sys.platform == 'darwin':
            volkshash_conf = os.path.join(home, "Library/Application Support/VolkshashCore/volkshash.conf")

    volkshash_conf = sentinel_cfg.get('volkshash_conf', volkshash_conf)

    return volkshash_conf
...
volkshash_conf = get_volkshash_conf()
network = get_network()
