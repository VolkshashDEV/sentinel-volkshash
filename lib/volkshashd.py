...
class VolkshashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some volkshashd methods
    ...
    @classmethod
    def from_volkshash_conf(self, volkshash_dot_conf):
        from volkshash_config import VolkshashConfig
        config_text = VolkshashConfig.slurp_config_file(volkshash_dot_conf)
        creds = VolkshashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = volkshashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = volkshashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = volkshashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_volkshashd_proto_version_with_sentinel_ping)
