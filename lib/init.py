...
def has_volkshash_conf():
    import config
    import io

    valid_volkshash_conf = False

    # ensure volkshash_conf exists & readable
    #
    # if not, print a message stating that Volkshash Core must be installed and
    # configured, including JSONRPC access in volkshash.conf
    try:
        f = io.open(config.volkshash_conf)
        valid_volkshash_conf = True
    except IOError as e:
        print(e)

    return valid_volkshash_conf
...
def main():
    ...
    if not has_volkshash_conf():
        print("VolkshashCore must be installed and configured, including JSONRPC access in volkshash.conf")
        sys.exit(1)
