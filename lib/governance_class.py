...
class GovernanceClass(object):
    ...
    def vote(self, volkshashd, signal, outcome):
        return self.go.vote(volkshashd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, volkshashd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(volkshashd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(volkshashd, models.VoteSignals.valid, models.VoteOutcomes.no)
