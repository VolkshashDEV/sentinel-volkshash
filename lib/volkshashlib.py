...
def is_valid_volkshash_address(address, network='mainnet'):
    ...
    volkshash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Volkshash address
        return False

    if (address_version != volkshash_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('volkshash.conf')
