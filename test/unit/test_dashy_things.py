...
def test_valid_volkshash_address():
    from volkshashlib import is_valid_volkshash_address

    main = valid_volkshash_address()
    test = valid_volkshash_address('testnet')

    assert is_valid_volkshash_address(main) is True
    assert is_valid_volkshash_address(main, 'mainnet') is True
    assert is_valid_volkshash_address(main, 'testnet') is False

    assert is_valid_volkshash_address(test) is False
    assert is_valid_volkshash_address(test, 'mainnet') is False
    assert is_valid_volkshash_address(test, 'testnet') is True


def test_invalid_volkshash_address():
    from volkshashlib import is_valid_volkshash_address

    main = invalid_volkshash_address()
    test = invalid_volkshash_address('testnet')

    assert is_valid_volkshash_address(main) is False
    assert is_valid_volkshash_address(main, 'mainnet') is False
    assert is_valid_volkshash_address(main, 'testnet') is False

    assert is_valid_volkshash_address(test) is False
    assert is_valid_volkshash_address(test, 'mainnet') is False
    assert is_valid_volkshash_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from volkshashlib import elect_mn
...
def test_parse_masternode_status_vin():
    from volkshashlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % volkshashlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(volkshashlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(volkshashlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(volkshashlib.blocks_to_seconds(16616)) == 2612035
